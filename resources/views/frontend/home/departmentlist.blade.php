...
<body>
<div class="container">
    <h1 style="text-transform: uppercase;">DANH SÁCH BÁC SĨ THEO KHOA: @foreach($department as $i)
                                {{ $i->id == $request->id ?
                                "$i->name" : '' }}
                                @endforeach</h1>
		...
		<div class="row">
				<div class="col-md-5 col-sm-12 col-xs-12">
					<div class="product-image">
						<img src="{{asset('public/'."$item->image")}}" class="img-responsive">
						<span class="tag2 hot">
							HOT
						</span>
					</div>
				...
				<div class="product-deatil">
						<h5 class="name">
							...
							<a href="#">
								<span>@foreach($region as $i)
                                {{ $i->id == $item->region_id ?
                                "$i->name" : '' }}
                                @endforeach</span>
							</a>
							<a href="#">
								<span>@foreach($hospital as $i)
                                {{ $i->id == $item->hospital_id ?
                                "$i->name" : '' }}
                                @endforeach</span>
							</a>
							<a href="#">
								<span>@foreach($department as $i)
                                {{ $i->id == $item->department_id ?
                                "$i->name" : '' }}
                                @endforeach</span>
							</a>
						...
						<p class="price-container">
							<span>@php echo number_format($item->price); @endphp đ</span>
						</p>
                         <a href="{{ route('site.home.show',$item->id)}}" class="btn btn-info">More info</a>
